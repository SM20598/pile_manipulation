...
from utilities.materials import spawn_sandbox
...
box_pos = (0.5, 0.0, 0.0) # center of the box
box_vol = (0.3, 0.3, 0.1)
wall_thickness = 0.02 # thickness of the box wall
granular_vol = (0.27, 0.27, 0.1)
material_type = "rsa" # granular, sand, or liquid. Default is granular (Random Sequential Addition)
material_properties = {"n_particles": 1}
particle_size = 0.05
...
def spawn_robot(
    ...
    if robot_name == 'franka':
        ...
        robot = scene.add_entity(
            gs.morphs.MJCF(
                file=os.path.join(pathlib.Path(__file__).parent.resolve(), 'utilities/xml/franka_emika_panda_with_tool/panda_robola_compile.xml'),
                pos=pos
            ),
        )
    else:
