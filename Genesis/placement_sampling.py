...
def nearest_free_placement(free: torch.Tensor, meta: dict, angles: torch.Tensor,
                           targets_xy: torch.Tensor,
                           max_candidates: int = 65536,
                           generator: torch.Generator | None = None):
    """Snap each target xy to the nearest collision-free tool placement.

    This is the composition of the two start samplers rather than a choice
    between them. Density-weighted sampling answers "where is there material
    worth pushing", which is a property of the *pile*; free-space sampling
    answers "where can the tool actually come down", which is a property of the
    *tool*. They are not alternatives, and taking either alone gives up
    something real: a density-weighted start can materialize the blade inside a
    cube (the solver resolves that by ejecting it, recorded as though it were a
    push), while a purely free-space start drifts toward empty tray, where a
    push moves nothing.

    So the neighbourhood is chosen by density and the exact pose is then moved
    the shortest distance that makes it legal. Displacement is typically a
    fraction of a particle diameter, so the density distribution is preserved
    up to that scale.

    Returns (xy [n_envs, n_samples, 2], yaw [n_envs, n_samples],
             ok [n_envs, n_samples] bool), with ok=False wherever an env's free
    set is empty - the same degradation path as ``sample_free_placements``.

    ``max_candidates`` bounds the free cells considered per env. A 128 mm tray
    at 1 mm resolution with 16 yaw bins is 262 144 (angle, cell) triples, and
    the exact nearest over all of them costs n_samples x that per env for no
    measurable gain over a large random subset - the free set is spatially
    dense wherever it is non-empty, so a subsample's nearest member is within a
    cell or two of the true nearest.
    """
    n_envs, n_angles, H, W = free.shape
    res = meta["resolution"]
    n_samples = targets_xy.shape[1]
    dev = free.device

    xy_out = torch.zeros((n_envs, n_samples, 2), device=dev, dtype=torch.float32)
    yaw_out = torch.zeros((n_envs, n_samples), device=dev, dtype=torch.float32)
    ok_out = torch.zeros((n_envs, n_samples), device=dev, dtype=torch.bool)
    angles = angles.to(dev)

    for e in range(n_envs):
        idx = free[e].reshape(-1).nonzero(as_tuple=False).squeeze(1)
        if idx.numel() == 0:
            continue                      # blocked env: caller keeps its draw
        if idx.numel() > max_candidates:
            pick = torch.randint(idx.numel(), (max_candidates,), device=dev,
                                 generator=generator)
            idx = idx[pick]

        a_i = torch.div(idx, H * W, rounding_mode="floor")
        rem = idx % (H * W)
        row = torch.div(rem, W, rounding_mode="floor")
        col = rem % W
        cx = (col.float() + 0.5) * res - meta["width"] / 2
        cy = (row.float() + 0.5) * res - meta["depth"] / 2
        cand = torch.stack((cx, cy), dim=-1)                  # [C, 2]

        d2 = (targets_xy[e].unsqueeze(1) - cand.unsqueeze(0)).pow(2).sum(-1)
        best = d2.argmin(dim=1)                               # [n_samples]
        xy_out[e] = cand[best]
        yaw_out[e] = angles[a_i[best]]
        ok_out[e] = True

    return xy_out, yaw_out, ok_out
