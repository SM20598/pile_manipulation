...
class SandboxManipulation:
    ...
    def __init__(
        ...
        # changing n_particles otherwise costs a full rebuild.
    ...
    def set_n_active(self, n: int) -> None:
        ...
        shape pays a full kernel recompile. Parking lets
    ...
    def shuffle_particles(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # paying a full rebuild per count.
    ...
    def _equalize_batch_travel(self, action_starts, action_stops, low, high):
        ...
        a large share of the batching penalty (see Genesis/action_sampling.py;
        the share is machine-specific - measure with
        tests/benchmarks/bench_performance.py). Start point, direction and
        blade yaw stay per-env, and the distance still varies from batch to
        batch, so only the within-batch spread is given up.
        """
    ...
    def generate_action_samples(
        ...
        shared_travel_distance: give every env the same push LENGTH for a given
            ...
            one's duration, which is a large share of the batching penalty.
            How large is machine-specific. Off by default so single-env and MPC
            callers are unaffected; collection turns it on. Pushes that cannot reach the shared distance without
