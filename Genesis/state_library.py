...
Usage
-----
    from state_library import build_state_library, StateLibrary
...
class StateLibrary:
    ...
    def apply(self, sim, rng=None, index: int | None = None) -> int:
        ...
        # Upstream's set_particle_state takes a packed (n_particles, 7) tensor
        # and broadcasts a 2D input across envs, which is exactly the shared
        # initial state this method wants.
        sim.set_particle_state(state)
        return index
    ...
    def apply_per_env(self, sim, rng=None, indices=None) -> list[int]:
        ...
        if states.shape[1] != len(sim.material):
            raise ValueError(
                ...
                f"{len(sim.material)}; libraries are specific to a build")
        # set_particle_state accepts a 3D (n_envs, n_particles, 7) tensor and
        # writes it per env unchanged.
        sim.set_particle_state(states.contiguous())
        return indices
...
def load_or_build_state_library(sim, path: str | Path, n_settles: int = 15, *,
    ...
    Settling is by far the most expensive thing in a run — a fresh
    respawn of 200 particles takes many hundreds of steps to come to rest,
