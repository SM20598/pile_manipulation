...
| `MPC/README.md` | the MPC stack's layout and how it reaches the simulator |
| `MPC/docs/ARCHITECTURE.md` | model/training/registry design |
...
> **Not in this branch.** The MPC stack is a separate contribution; this
> section describes it so the simulator's extension points make sense. The
> paths below refer to that companion tree.
...
`MPC/docs/ARCHITECTURE.md` (module map, data flow,
extension points, config structure),
`MPC/docs/INTERFACES.md` (the contracts between model,
dataset and trainer),
`MPC/docs/UTILITIES.md`,
`MPC/docs/oracle_mpc_design.md` (why the oracle exists
and what it records),
`MPC/docs/human_demo_design.md`.
