...
class _FakeSim:
    ...
    def set_particle_state(self, state):
        """Upstream's signature: one packed (.., n_particles, 7) tensor.

        A 2D input is broadcast across envs by the real implementation, so the
        library deliberately hands it a 2D state for the shared-reset path.
        """
        self.written = state
...
def test_apply_broadcasts_one_state_to_every_env():
    ...
    assert idx == 2
    state = sim.written
    # a single un-batched state, left for set_particle_state to broadcast
    assert state.shape == (9, 7)
    assert torch.allclose(state[:, 0:3], lib.states[2, :, 0:3])
    assert torch.allclose(state[:, 3:7], lib.states[2, :, 3:7])
